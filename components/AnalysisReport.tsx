  ...
  <li className="flex items-start space-x-3 py-3">
    ...
    <div className="flex-1">
      <p className="text-sm font-semibold text-red-700 dark:text-red-400">
        <span className="bg-red-100 dark:bg-red-900/50 px-2 py-1 rounded-md">{anomaly.timestamp}</span>
      </p>
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md border border-gray-200 dark:border-gray-700 overflow-hidden">
      <div className="p-4 bg-gray-50 dark:bg-gray-700/50 border-b border-gray-200 dark:border-gray-700 flex flex-col sm:flex-row sm:items-center sm:justify-between">
        <h3 className="text-lg font-bold text-gray-800 dark:text-gray-200 flex items-center mb-2 sm:mb-0">
          <FileVideoIcon className="h-5 w-5 mr-3 text-gray-500" />
          {result.fileName}
        </h3>
        <div className="text-sm">{getStatusIndicator()}</div>
      </div>

      {result.status === 'completed' && result.anomalies.length > 0 && (
        <div className="p-4">
      ...
      {result.status === 'error' && (
        <div className="p-6 text-center text-yellow-700 dark:text-yellow-300 bg-yellow-50 dark:bg-yellow-900/20">
